...
class MemoryBuilder:
    ...
    def __init__(
        ...
        Args:
            ...
            llm_model: LLM model name (e.g., "gpt-4o-mini", "gpt-4o", "qwen2.5:3b", "qwen2.5:7b")
            use_episodes: Whether to use episode-based segmentation
            embedding_model: Embedding model to use
            llm_backend: Backend to use ("openai" for GPT models or "ollama" for local models like Qwen)
        """
        import os
        import sys
        ...
        # Configure LLM controller based on backend
        ...
        if llm_backend == "ollama":
            ...
            logger.info(f"Using Ollama backend with model: {llm_model}")
        else:
            # Check for Azure or OpenAI API key
            ...
            if use_azure:
                ...
                if not api_key:
                    logger.warning(
                        ...
                        "\n- Set AZURE_OPENAI_API_KEY environment variable for better results" +
                        "\n" + "="*60
                    )
            else:
        ...
        if self.llm_controller and hasattr(self.llm_controller, 'llm'):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    def _to_str(v):
                        """Coerce LLM output to string — handles ints, dicts, etc."""
                        if isinstance(v, dict):
                            return v.get('name', v.get('text', v.get('value', str(v))))
                        return str(v)

                    entities = [_to_str(e) for e in extracted.get('entities', [])]
                    topic = str(extracted.get('topic', 'general'))
                    dates_mentioned = [_to_str(d) for d in extracted.get('dates_mentioned', [])]

                    if 'summary' in extracted:
                        content_parts.insert(0, f"Summary: {extracted['summary']}")

                    semantic_facts = [_to_str(f) for f in extracted.get('semantic_facts', [])]
                    relationships = [_to_str(r) for r in extracted.get('relationships', [])]
                    activities = [_to_str(a) for a in extracted.get('activities', [])]
                    context_keywords = [_to_str(k) for k in extracted.get('context_keywords', [])]
