...
class QueryEngine:
    ...
    def __init__(self, trg_memory, node_index: dict, entity_session_map: dict = None, entity_dia_map: dict = None, llm_controller=None, ablation_config=None, edge_scorer: EdgeScorerInterface = None):
        """
        ...
        Args:
            ...
            edge_scorer: Optional edge scorer for graph traversal
                       If None, uses StaticEdgeScorer (baseline HAGE behavior)
        """
        self.trg = trg_memory
        ...
        # Initialize edge scorer (default to static baseline)
        if edge_scorer is None:
            self.edge_scorer = StaticEdgeScorer()
            logger.info("QueryEngine using StaticEdgeScorer (baseline)")
        else:
            self.edge_scorer = edge_scorer
            logger.info(f"QueryEngine using {type(edge_scorer).__name__}")
    ...
    def extract_date_from_question(question: str):
        ...
        # Pattern 1a: Month DD, YYYY (e.g., "March 16, 2022")
        pattern1a = rf'({month_names})\s+(\d{{1,2}}),?\s+(20\d{{2}})'
        match = re.search(pattern1a, question, re.IGNORECASE)
        if match:
            month_name, day, year = match.groups()
            return {'year': int(year), 'month': month_map[month_name.lower()], 'day': int(day)}

        # Pattern 1b: DD Month, YYYY (e.g., "1 May, 2022")
        pattern1b = rf'(\d{{1,2}})\s+({month_names}),?\s+(20\d{{2}})'
        match = re.search(pattern1b, question, re.IGNORECASE)
        if match:
    ...
    def query(self, question: str, top_k: int = 15) -> Tuple[QueryContext, str]:
        ...
        # For any question containing a date, inject timestamp-matching nodes
        # into the candidate pool after RRF fusion. This helps even when the
        # query_type is 'activity' (e.g., "What movie did X watch on 1 May?").
        temporal_match_ids = set()
        _temporal_inject_nodes = []
        _has_date = self.extract_date_from_question(question) is not None
        if query_type == 'temporal' or _has_date:
            temporal_nodes = self._timestamp_search(question, max_results=30)
            temporal_match_ids = {n.node_id for n in temporal_nodes}

            # Filter to nodes with keyword overlap
            question_lower = question.lower()
            stop_words = {'the', 'a', 'an', 'is', 'was', 'are', 'were', 'what', 'when',
                         'where', 'who', 'how', 'did', 'does', 'do', 'in', 'on', 'at',
                         'to', 'for', 'of', 'and', 'or', 'his', 'her', 'its'}
            import re as _re
            query_keywords = [_re.sub(r'[^\w]', '', w) for w in question_lower.split()]
            query_keywords = [w for w in query_keywords if len(w) > 2 and w not in stop_words]
            for tn in temporal_nodes:
                content = (tn.content_narrative if hasattr(tn, 'content_narrative') else str(tn)).lower()
                keyword_hits = sum(1 for kw in query_keywords if kw in content)
                if keyword_hits >= 2:
                    tn.similarity_score = 0.6
                    _temporal_inject_nodes.append(tn)
            logger.info(f"Timestamp search: {len(_temporal_inject_nodes)} keyword-filtered nodes for injection (from {len(temporal_nodes)} date matches)")
        ...
        # Inject timestamp-matched nodes that aren't already candidates
        if _temporal_inject_nodes:
            injected = 0
            for tn in _temporal_inject_nodes:
                if tn.node_id not in existing_ids:
                    all_candidates.append(tn)
                    existing_ids.add(tn.node_id)
                    injected += 1
            if injected:
                logger.info(f"Injected {injected} timestamp-matched nodes into candidate pool")
        ...
        if query_type == 'multi_hop' and all_candidates:
            ...
            if all_candidates:
                top_nodes = self._rerank_and_filter(
                    ...
                    scoring_weights=adaptive_params.get('scoring_weights', {}),
                    temporal_match_ids=temporal_match_ids
                )
            else:
                top_nodes = []
    ...
    def _timestamp_search(self, question: str, max_results: int = 30) -> List[EventNode]:
        """
        Retrieve nodes by timestamp matching for temporal queries.

        Extracts date references from the question and finds nodes whose
        timestamps fall within a matching window. This compensates for
        embedding-based search failing to capture date specifics.
        """
        import re
        from datetime import datetime, timedelta
        ...
        # Extract year, month, month-name from question
        months_map = {
            'january': 1, 'february': 2, 'march': 3, 'april': 4,
            'may': 5, 'june': 6, 'july': 7, 'august': 8,
            'september': 9, 'october': 10, 'november': 11, 'december': 12
        ...
        # Try full date: "1 May, 2022" or "May 1, 2022"
        date_info = self.extract_date_from_question(question)
        if date_info:
            target_year = date_info.get('year')
            target_month = date_info.get('month')
            target_day = date_info.get('day')

        # Try month name without full date
        ...
        # Search all nodes by timestamp
        all_nodes = list(self.trg.graph_db.nodes.values()) if hasattr(self.trg, 'graph_db') else []
        ...
        for node in all_nodes:
            ...
            try:
                if isinstance(ts, str):
                    node_dt = datetime.fromisoformat(ts.replace('Z', ''))
            ...
            # Match year
            if target_year and node_dt.year != target_year:
                continue

            # Match month (if specified)
            if target_month and node_dt.month != target_month:
                continue

            # Match day with ±2 day window (if specified)
            if target_day:
                if abs(node_dt.day - target_day) > 2:
                    continue

            # Filter by person name if present
            content = ''
            ...
            if person_names and not any(name in content for name in person_names):
                continue

            # Score: tighter date match = higher score
            score = 0.5
            if target_day:
                day_diff = abs(node_dt.day - target_day)
                score = 0.9 - day_diff * 0.15  # exact=0.9, ±1=0.75, ±2=0.6
            elif target_month:
                score = 0.7
            else:
                score = 0.5
        ...
        matched.sort(key=lambda n: getattr(n, 'similarity_score', 0), reverse=True)

        logger.info(f"Timestamp search: year={target_year}, month={target_month}, day={target_day}, "
                     f"persons={person_names}, found {len(matched)} nodes")
    ...
    def _adaptive_graph_traversal(
        ...
        while queue and len(result_nodes) < max_nodes:
            ...
            # Get neighbors WITH edge types for scoring
            neighbors_with_types = self._get_neighbors_with_edge_types(current_node, follow_link_types=prefer_link_types)

            # Filter by keywords
            promising = [(n, etype) for n, etype in neighbors_with_types
            ...
            for neighbor, edge_type in promising[:neighbor_limit]:
                ...
                if neighbor_content:
                    ...
                    # CORE CHANGE: Use edge scorer instead of just semantic similarity
                    edge_score = self.edge_scorer.score_edge(
                        source_node=current_node,
                        target_node=neighbor,
                        edge_type=edge_type,
                        query_text=question,
                        query_embedding=query_embedding,
                        context={'depth': depth, 'parent_similarity': current_sim}
                    )

                    # Combine semantic similarity with edge score
                    # Edge score acts as a gating/weighting mechanism
                    combined_score = neighbor_sim * edge_score

                    # Log high-scoring edges for debugging
                    if edge_score > 0.7:
                        logger.debug(
                            f"High edge score: {edge_score:.3f} | "
                            f"Type: {edge_type.value} | "
                            f"Sim: {neighbor_sim:.3f} | "
                            f"Combined: {combined_score:.3f} | "
                            f"Target: {neighbor_content[:60]}..."
                        )

                    # Use combined score for traversal decisions
                    if combined_score >= similarity_threshold * 0.5:  # Lower threshold since we're multiplying
                        drop = current_sim - combined_score
                        if drop <= relative_drop_threshold:
                            queue.append((neighbor, combined_score, depth + 1, current_sim))

        result_nodes.sort(key=lambda x: x[1], reverse=True)
        return result_nodes

    def _get_neighbors_with_edge_types(self, node: EventNode, follow_link_types: Optional[Set[LinkType]] = None) -> List[Tuple[EventNode, LinkType]]:
        """
        Get neighbors through graph links WITH their edge types.

        Returns:
            List of (neighbor_node, edge_type) tuples
        """
        neighbors_with_types = []
        ...
        for link in self.trg.graph_db.links.values():
            ...
            if follow_link_types and link.link_type not in follow_link_types:
                # Allow some important subtypes even if link type doesn't match
                if subtype not in ['CONTEXT_NEIGHBOR', 'SAME_ENTITY', 'TEMPORALLY_CLOSE']:
                    continue
            ...
            if target and target.node_id != node.node_id and target.node_id not in seen:
                neighbors_with_types.append((target, link.link_type))
                seen.add(target.node_id)

        return neighbors_with_types
    ...
    def _rerank_and_filter(
        ...
        for node in nodes:
            ...
            # Boost nodes whose timestamps match the query date
            if temporal_match_ids and node.node_id in temporal_match_ids:
                score += 15.0
