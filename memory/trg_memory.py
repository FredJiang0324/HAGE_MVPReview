...
class TemporalResonanceGraphMemory:
    ...
    def __init__(self,
        ...
        if LLM_AVAILABLE and llm_backend:
            try:
                # Get API key from environment for OpenAI or Azure
                api_key = None
                actual_model = llm_model  # Use the passed model by default
                if llm_backend == 'openai':
                    import os
                    # Check for Azure or regular OpenAI
                    use_azure = os.getenv("USE_AZURE_OPENAI", "false").lower() == "true"
                    if use_azure:
                        api_key = os.getenv('AZURE_OPENAI_API_KEY')
                        # For Azure, use deployment name if model is default
                        if llm_model == 'gpt-4o-mini':
                            actual_model = os.getenv('AZURE_OPENAI_DEPLOYMENT_NAME', 'gpt-4.1-mini')
                    else:
    ...
    def _extract_event(self, content: str,
        ...
        if self.llm_controller:
            ...
            try:
                ...
                # Handle response that might be a string or already parsed
