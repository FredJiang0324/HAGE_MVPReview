...
class OpenAIController(BaseLLMController):
    def __init__(self, model: str = "gpt-4", api_key: Optional[str] = None):
        try:
            ...
            # Use provided model or get from environment/Azure deployment
            self.model = model if model else get_model_name("gpt-4")
            # Get appropriate client (OpenAI or Azure)
...
class OllamaController(BaseLLMController):
    ...
    def get_completion(self, prompt: str, response_format: dict, temperature: float = 0.7) -> str:
        try:
            # Extract the expected JSON structure from response_format
            expected_structure = ""
            if "json_schema" in response_format:
                schema = response_format["json_schema"]["schema"]
                if "properties" in schema:
                    # Create a sample JSON structure to guide the model
                    sample = {}
                    for prop, details in schema["properties"].items():
                        if details["type"] == "array":
                            sample[prop] = ["example1", "example2"]
                        elif details["type"] == "string":
                            sample[prop] = "example_string"
            ...
            full_prompt = f"""You MUST respond with ONLY a valid JSON object. Do not include any text before or after the JSON.
...
Remember: Output ONLY valid JSON, nothing else."""
            ...
            # Use Ollama's chat API with JSON-specific instructions
            response = self.client.chat(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a JSON API that only responds with valid JSON objects. Never include explanatory text."},
                    {"role": "user", "content": full_prompt}
                ],
                options={
                    "temperature": temperature,
                    "format": "json"  # Hint to Ollama to use JSON format if supported
                }
            )
            ...
            try:
                ...
                # Return empty response if we can't extract valid JSON
                empty_response = self._generate_empty_response(response_format)
                return json.dumps(empty_response)
